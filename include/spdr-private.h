...
#  define UU_SPDR_INT(argkey, argvalue)   \
	((struct SPDR_Event_Arg) { .key = argkey, .type = SPDR_INT, .value = { .i = argvalue } })
#  define UU_SPDR_FLOAT(argkey, argvalue) \
	((struct SPDR_Event_Arg) { .key = argkey, .type = SPDR_FLOAT, .value = { .d = argvalue } })
#  define UU_SPDR_STR(argkey, argvalue) \
	((struct SPDR_Event_Arg) { .key = argkey, .type = SPDR_STR, .value = { .str = argvalue } })
#else
...
#    define UU_SPDR_SCOPE_SETUP(spdr, cat, name)			\
	struct SPDR_Scope UU_SPDR_CONCAT(scope,__LINE__)			\
	  __attribute__((cleanup(SPDR_Scope_exit))) = { spdr, cat, name }; (void) UU_SPDR_CONCAT(scope,__LINE__)
